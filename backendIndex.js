...
app.get("/api/mint/:account/:amount/:adminAccount/:adminCvv", async (req,res) => {
	...
	if(!req.params.adminAccount){
		return res.send("Falta el administrador que autorizo la impresion de los fondos.")
	}
...
app.get("/api/burn/:account/:amount/:adminAccount/:adminCvv", async (req,res) => {
	if(!req.params.account){
		return res.send("Falta la cuenta que perdera los fondos")
	}

	if(!req.params.amount){
		return res.send("Falta el monto a eliminar.")
	}
	...
	res.send(await burn.burn(account,amount,adminAccount,adminCvv))
